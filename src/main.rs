...
impl<'a> Editor<'a> {
    ...
    pub fn update_operators(&'a mut self) {
        let ops = get_operators(&self.wasm_bin);

        let mut ops_iter = ops.into_iter();
        for line in self.lines.iter_mut() {
            if line.activated() && *(line.info()) != InstrInfo::EmptyorMalformed {
                line.set_op(ops_iter.next());
            }
        }
    }

    pub fn input(&'a mut self, idx: usize, new_text: &str) {
...
#[derive(Debug)]
pub struct EditLine<'a> {
    id: usize,
    text: String,
    //div_ref: DivRef,
    info: InstrInfo,
    activated: bool,
    op: Option<wasmparser::Operator<'a>>,
}
...
impl<'a> EditLine<'a> {
    ...
    pub fn op(&self) -> &Option<wasmparser::Operator<'a>> {
        &self.op
    }

    pub fn set_op(&mut self, operator: Option<wasmparser::Operator<'a>>) {
        self.op = operator;
    }
}
